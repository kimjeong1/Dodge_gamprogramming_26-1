...
class DashTrailRenderer : public Component
{
    ...
    std::mt19937                            rng;
    std::uniform_real_distribution<float>   randAngle;   // 파티클 방향
    std::uniform_real_distribution<float>   randSpeed;   // 파티클 속도
    std::uniform_int_distribution<int>      randSize;    // 파티클 크기

    // 마지막 대시 방향 (PlayerController 에서 설정)
    float dashDirX = 0.0f;
    float dashDirY = 0.0f;

    static const int   AFTERIMAGE_COUNT;   // 잔상 몇 개 찍을지
    static const float AFTERIMAGE_INTERVAL;// 잔상 간격 (초)
    static const float AFTERIMAGE_LIFE;    // 잔상 수명
    static const int   PARTICLE_COUNT;     // 파티클 개수
    static const float PARTICLE_LIFE;      // 파티클 수명
...
public:
    ...
    // PlayerController 가 대시 발동 직후 호출
    void OnDash(float startX, float startY,
    ...
    void Render(GraphicsContext*) override
    {
        ...
        if (afterimages.empty() && particles.empty()) return;

        HDC hdc = GetDC(hWnd);

        // ── 잔상 렌더링 (함선 실루엣 삼각형 + 날개) ─────────
        for (auto& img : afterimages)
        {
            // alpha 0~1 → RGB 밝기로 흉내 (GDI 는 알파 미지원)
            // 배경색(거의 검정)과 블렌딩: 색상 * alpha
            int brightness = (int)(img.alpha * 255);
            int r = (int)(0.3f * brightness);   // 하늘색 계열
            int g = (int)(0.8f * brightness);
            int b = brightness;
            ...
            // 함선 실루엣 (CreateShipMesh 와 동일한 비율, 픽셀 단위)
            float hw = 12.0f, hh = 12.0f;
            // 몸체 삼각형
            POINT body[3] = {
                { (LONG)(img.x),        (LONG)(img.y - hh * 2.0f) },
                { (LONG)(img.x + hw),   (LONG)(img.y + hh) },
                { (LONG)(img.x - hw),   (LONG)(img.y + hh) }
            };
            Polygon(hdc, body, 3);
            ...
            // 왼쪽 날개
            POINT wingL[3] = {
                { (LONG)(img.x),           (LONG)(img.y + hh * 0.5f) },
                { (LONG)(img.x - hw),      (LONG)(img.y + hh) },
                { (LONG)(img.x - hw * 2.5f), (LONG)(img.y + hh * 1.5f) }
            };
            Polygon(hdc, wingL, 3);
        ...
        for (auto& p : particles)
        {
            int brightness = (int)(p.alpha * 255);
            // 하늘색 → 흰색으로 밝게
            int r = brightness;
            int g = brightness;
            int b = brightness;
            ...
            RECT rc = {
                (LONG)(p.x - p.size), (LONG)(p.y - p.size),
                (LONG)(p.x + p.size), (LONG)(p.y + p.size)
            };
...
public:
    BlinkRenderer(ColorMaterial* mat) 
        : pMat(mat)
        , rng(std::random_device{}())
        , randCol(0.3f, 1.0f)   // 너무 어둡지 않게 0.3 ~ 1.0
    {}
...
public:
    ShieldRenderer(HWND hwnd) : hWnd(hwnd) {}
    ...
    void Render(GraphicsContext*) override
    {
        ...
        int R = 255;
        int G = (int)(200.0f * ratio + 50.0f);
        int B = (int)(50.0f * ratio);

        static const float TWO_PI = 6.28318530718f;  // XM_2PI 대신

        for (int layer = 0; layer < 2; ++layer)
        {
            float lr = r + layer * 4.0f;
            int dimR = (layer == 0) ? R : R / 2;
            int dimG = (layer == 0) ? G : G / 2;
            int dimB = (layer == 0) ? B : B / 2;

            HPEN   pen = CreatePen(PS_SOLID, (layer == 0) ? 2 : 1, RGB(dimR, dimG, dimB));
            HPEN   oldPen = (HPEN)SelectObject(hdc, pen);
            ...
            for (int i = 0; i <= SIDES; ++i)
            {
                ...
                pts[i].x = (LONG)(pOwner->posX + cosf(angle) * lr);
                pts[i].y = (LONG)(pOwner->posY + sinf(angle) * lr);
            }
            Polyline(hdc, pts, SIDES + 1);
        ...
        {
            float angle = rotAngle + (i * TWO_PI / SIDES);
            int cx = (int)(pOwner->posX + cosf(angle) * r);
            int cy = (int)(pOwner->posY + sinf(angle) * r);

            HBRUSH dotBrush = CreateSolidBrush(RGB(255, 255, 180));
...
int WINAPI WinMain(HINSTANCE hI, HINSTANCE, LPSTR, int)
{
    ...
    auto* trail = new DashTrailRenderer(gEngine.win.hWnd);
