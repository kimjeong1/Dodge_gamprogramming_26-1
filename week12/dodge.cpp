...
public:
    ...
    void Render()
    {
        ...
        else if (gState == GameState::GAMEOVER)
        {
            ...
            DrawText(L"R  :  재시작", 510, 390, RGB(200, 200, 200));
            DrawText(L"ESC  :  종료", 510, 430, RGB(200, 200, 200));
        }
    }
...
public:
    ...
    void SpawnObstacle()
    {
        if (!obstacles || !gfx || !obsMesh || !obsMat) return;
    ...
    void Update(float dt)
    {
        ...
        // 장애물 스폰
        spawnTimer += dt;
        if (spawnTimer >= OBS_SPAWN_INTERVAL) { SpawnObstacle(); spawnTimer = 0.0f; }
...
int WINAPI WinMain(HINSTANCE hI, HINSTANCE, LPSTR, int)
{
    ...
    Mesh* playerMesh = CreateRectMesh(gEngine.gfx.Device, PLAYER_HALF_W, PLAYER_HALF_H);
    Mesh* obsMesh = CreateRectMesh(gEngine.gfx.Device, OBS_HALF_W, OBS_HALF_H);
    ...
    ColorMaterial* playerMat = new ColorMaterial(shaders, { 1.0f, 1.0f, 1.0f, 1.0f }, gEngine.gfx.Device); // 흰색
    ColorMaterial* obsMat = new ColorMaterial(shaders, { 1.0f, 0.3f, 0.3f, 1.0f }, gEngine.gfx.Device); // 빨간색
