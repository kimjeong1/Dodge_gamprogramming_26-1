...
■ 대시 스킬 (스페이스바)
  - 이동 거리: 180px (현재 이동 방향으로 순간이동)
  - 쿨타임: 3초
  - 화면 좌측 하단 게이지 바로 쿨타임 시각화
  - 대시 시 잔상(DashTrail) 이펙트 표시

■ 장애물 (운석)
  - 크기: 18x18px
  - 등장 위치: 화면 4방향 가장자리(상/하/좌/우) 중 랜덤
  - 이동 방향: 화면 안쪽으로 진입, 대각선 방향 랜덤 편차 포함
...
■ 무적 아이템 (별)
  ...
  - 효과: 먹으면 5초간 무적 (충돌 판정 무시)
  - 무적 중 플레이어 깜빡임 이펙트 표시

■ 난이도 시스템
  - 20초마다 자동으로 난이도 1단계 상승 (최대 6단계)
  - 단계별 변화:
    * 장애물 속도 증가: 단계당 최소 +40px/s, 최대 +60px/s
    * 스폰 간격 감소: 최대 난이도에서 기본의 약 33% 수준으로 빨라짐
    * 스폰 수 증가: 기본 4개 + 난이도 단계 수 (6단계 시 10개)

  | 난이도 | 경과 시간 | 속도 범위 | 스폰 간격 | 1회 스폰 수 |
  |--------|----------|-----------|----------|------------|
  | 0단계  | 0초~     | 150~380   | 1.2초    | 4개        |
...
----------------------------------------------------------------------------------------------------------
3. 렌더링 파이프라인 및 최적화 (Rendering Pipeline)
