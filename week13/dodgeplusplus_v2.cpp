...
--------------------------------------------------------------------------------
 [14주차 변경 사항]
 - TextRenderer (일반 클래스) 해체 → Component 2개로 분리
     StarBackgroundRenderer : 별 배경 스크롤 (PrimitiveBatch)
     UITextRenderer         : 비트맵 폰트 UI (DirectX 텍스처 쿼드)
 - GameManager (일반 클래스) → GameManagerComponent
 - 씬 오브젝트 추가
     gSystemObj : GameManagerComponent 부착
     gUIObj     : StarBackgroundRenderer 부착
     gUITextObj : UITextRenderer 부착 (Flush 이후 Draw 순서 보장)
 - GameLoop 의 manager / text 멤버 제거 → Component 로 이전
 - 모든 시스템이 Start / Input / Update / Render 인터페이스 준수
...
public:
    ...
    bool Initialize(HINSTANCE hInst, LRESULT(CALLBACK* wndProc)(HWND, UINT, WPARAM, LPARAM))
    {
        ...
        hWnd = CreateWindow(L"DX11Engine", L"Dodge++ | Week 14",
